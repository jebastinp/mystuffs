...
# Full table paths
RAW_BQ_TABLE = f"{PROJECT_ID}.{DATASET_ID}.{RAW_TABLE_NAME}"
TRANSFORMED_BQ_TABLE = f"{PROJECT_ID}.{DATASET_ID}.{TRANSFORMED_TABLE_NAME}"
...
QUERY = f"""
    ...
    SELECT
        customer_nbr,
        customer_desc,
        PARSE_DATETIME('%Y-%m-%d %H:%M:%S', start_ts) AS start_ts,
        PARSE_DATETIME('%Y-%m-%d %H:%M:%S', end_ts) AS end_ts,
        PARSE_DATETIME('%Y-%m-%d %H:%M:%S', create_ts) AS create_ts,
        create_user_id,
        PARSE_DATETIME('%Y-%m-%d %H:%M:%S', last_update_ts) AS last_update_ts,
        last_update_user_id,
