...
OUTPUT_PATH = "gs://casestudy2testing/processed/" # Ensure trailing slash for directory output
 
# Define expected schema (optional but highly recommended for robust parsing)
schema = StructType([
    StructField("store_id", StringType(), True),
    StructField("category_id", StringType(), True),
    StructField("quantity", IntegerType(), True), # Assuming quantity is integer
    StructField("price", DoubleType(), True),    # Assuming price can be decimal
    StructField("timestamp", StringType(), True), # Read as String, then parse to_date
    StructField("transaction_id", StringType(), True), # Assuming other fields might exist
    StructField("customer_id", StringType(), True)
    # Add any other fields from your JSON data here with their correct types
])
 
# --- Main PySpark Logic ---
if __name__ == "__main__":
    print("PySpark script 'transform.py' started.")
    ...
    try:
        ...
        # Set Spark log level to WARN to reduce verbosity in console logs
        ...
        try:
            ...
            df.printSchema() # This is CRITICAL for debugging now
            print("-------------------------------------------------\n")
 
            print(f"Number of records read: {df.count()}") # Counts rows - be cautious on very large datasets
        ...
        # Check if DataFrame is empty after reading (or if all records were corrupt)
        if "_corrupt_record" in df.columns and df.filter(col("_corrupt_record").isNotNull()).count() > 0:
            ...
            # You might want to inspect these corrupt records or handle them specifically.
        ...
        if df.isEmpty() or (len(df.columns) == 1 and "_corrupt_record" in df.columns and df.filter(col("_corrupt_record").isNotNull()).count() == df.count()):
            ...
            sys.exit(0) # Exit with success if no valid data was found, as no error in script logic happened.
        ...
        # Use a more robust way to stop the Spark session
        if spark: # Check if spark session was even initialized
            try:
                spark.stop()
    ...
    print("PySpark script 'transform.py' finished.")